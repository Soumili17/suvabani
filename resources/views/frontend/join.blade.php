...
@section('title', 'Membership Form | SUVABANI FOUNDATION')
...
<div class="header">
    ...
    <div class="details">
        <h2>SUVABANI FOUNDATION ( শুভাবনী ফাউন্ডেশন )</h2>
        <p>Govt. Registration No. IV-160300107/2025</p>
        <p>Rania Pravat Pally, P.O. Boral, P.S. Narendrapur, Kolkata- 700 154</p>
        <p>Contact No.: 7059590022 | Email: suvabanifoundation@gmail.com</p>
    </div>
</div>
...
<form id="membershipForm" action="{{ route('membership.submit') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <fieldset>
        ...
        <input type="file" id="idfile" name="idfile" accept=".pdf,.jpg,.jpeg,.png" required>
    ...
    <fieldset>
        ...
        <label><input type="checkbox" name="membership[]" value="Paid" required> Paid - Rs. 200 or More Rs. <input type="number" name="paidamount" min="200"></label>
        <label><input type="checkbox" name="membership[]" value="Non-Paid"> Non-Paid</label>

        <label>Type of Membership:</label>
        <input type="checkbox" name="membertype[]" value="General Member" required> General Member
    ...
    <fieldset>
        ...
        <input type="checkbox" name="interest[]" value="Child Education" required> Child Education
...
document.querySelectorAll('#nextBtn').forEach(btn=>{
    btn.addEventListener('click', ()=>{
        if(validateStep(current) && current<fieldsets.length-1){ current++; showStep(current); }
    });
});
...
function validateStep(step){
    ...
    switch(step){
        case 0: if(!form.photo.files.length){ alert('Please upload your photo.'); return false; } break;
        case 1:
            if(form.fullname.value.trim()===''){ alert('Full Name required'); return false; }
            if(form.parentname.value.trim()===''){ alert('Parent/Spouse Name required'); return false; }
            if(form.dob.value===''){ alert('DOB required'); return false; }
            if(!form.querySelector('input[name="gender"]:checked')){ alert('Select gender'); return false; }
            if(form.nationality.value.trim()===''){ alert('Nationality required'); return false; }
            if(form.occupation.value.trim()===''){ alert('Occupation required'); return false; }
            if(form.address.value.trim()===''){ alert('Address required'); return false; }
            if(form.phone.value.trim()===''){ alert('Phone required'); return false; }
            if(!/^\d{10}$/.test(form.phone.value)){ alert('Phone must be 10 digits'); return false; }
            if(form.email.value.trim()===''){ alert('Email required'); return false; }
            if(!/^\S+@\S+\.\S+$/.test(form.email.value)){ alert('Enter valid email'); return false; }
            break;
        case 2:
            if(!form.querySelector('input[name="idproof[]"]:checked')){ alert('Select at least one ID proof'); return false; }
            if(form.idnumber.value.trim()===''){ alert('ID Number required'); return false; }
            if(!form.idfile.files.length){ alert('Upload ID proof'); return false; }
            break;
        case 3:
            if(!form.querySelector('input[name="membership[]"]:checked')){ alert('Select at least one membership'); return false; }
            if(!form.querySelector('input[name="membertype[]"]:checked')){ alert('Select at least one member type'); return false; }
            break;
        case 4: if(!form.querySelector('input[name="interest[]"]:checked')){ alert('Select at least one interest'); return false; } break;
        case 6:
            if(!form.signature.files.length){ alert('Upload signature'); return false; }
            if(form.declaration_date.value===''){ alert('Select declaration date'); return false; }
            break;
    }
    return true;
}
...
document.getElementById('membershipForm').addEventListener('submit', function(e){
    e.preventDefault();
    for(let i=0;i<fieldsets.length;i++){ if(!validateStep(i)){ current=i; showStep(current); return false; } }
    this.submit();
});
...
document.getElementById('photo').addEventListener('change', function(){
    ...
    if(file){ const preview = document.getElementById('photoPreview'); preview.src=URL.createObjectURL(file); preview.style.display='block'; }
});

// Preview signature
document.getElementById('signature').addEventListener('change', function(){
    const [file] = this.files;
    if(file){ const preview = document.getElementById('signaturePreview'); preview.src=URL.createObjectURL(file); preview.style.display='block'; }
});
</script>
@endpush
